...
import pandas as pd
df = pd.read_csv(os.path.join(os.path.dirname(__file__), "datasets", "dataset_clean.csv"), low_memory=False)

# subsample 2000 uzoraka (1400 normal, 600 sqli — 70/30)
normal = df[(df["label"] == 0) & (df["split"] == "test")].sample(n=7000, random_state=42)
sqli   = df[(df["label"] == 1) & (df["split"] == "test")].sample(n=3000, random_state=42)
...
ax.set_title("Isolation Forest — Anomaly Score po uzorku", fontsize=13)
